...
#define LOG_OUT                 // Выводить логирование, увеличивает размер прошивки
...
struct BMSConfig {
    ...
    bool modeNum = false;                      // Нумерация запросов: true-включена, false-отключена
    ...
    uint16_t rxBufferSize = 320;               // Размер буфера приема UART
    unsigned long timeoutMs = 10;              // Таймаут приема (мс)
    bool modeFastCorrect = true;               // Ускоренная коррекция количества ячеек: true-включена, false-отключена
    uint8_t correctIterations = 3;             // Количество итераций коррекции
    bool modeErrorLimit = true;                // Ограничение по количеству ошибочных пакетов: true-включено, false-отключено
    ...
    unsigned long readRequestInterval = 1000;  // Интервал запроса чтения всех регистров
    unsigned long sendCommandInterval = 150;   // Интервал отправки команды, не чаще 100 мс
};
...
extern uint8_t *onCharge;     // Включить зарядные MOSFET
extern uint8_t *offCharge;    // Выключить зарядные MOSFET
extern uint8_t *onDischarge;  // Включить разрядные MOSFET
extern uint8_t *offDischarge; // Выключить разрядные MOSFET
extern uint8_t *onBalancer;   // Включить балансир
extern uint8_t *offBalancer;  // Выключить балансир
